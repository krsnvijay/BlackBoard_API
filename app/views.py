...
from app.serializers import FacultySerializer, ClassSerializer, ScheduleSerializer, AvailabilitySerializer
...
class AvailabiltyViewSet(APIView):
    ...
    def get_object(self, dept, day,hour):
        try:
            set1 = Faculty.objects.filter(dept=dept,faculty_schedule__day=day, faculty_schedule__hour=hour).values('faculty_id')
            set2 = Faculty.objects.filter(dept=dept).exclude(faculty_id__in=set1).values('faculty_id','name','phone')
            return set2

            # return set2.difference(set1).values('faculty_id','faculty_id__name','faculty_id__phone')
            # set1= Faculty.objects.filter(dept=dept, faculty_schedule__day=day,faculty_schedule__hour =hour)
            # set2= Faculty.objects.filter(dept=dept)
    ...
    def get(self, request):
        ...
        # serializer = AvailabilitySerializer(availability, context={'request': request})
        # return Response(serializer.data)